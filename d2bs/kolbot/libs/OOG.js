...
MainLoop:
	...
	isCharSafe: function (name) {
		var control, text, string, days, p;

		if (getLocation() === 12) { //Get char screen text
			control = getControl(4, 37, 178, 200, 92);
			...
			if (control) {
				do {
					...
					if (text instanceof Array && typeof text[1] === "string" && text[1].toLowerCase() == name.toLowerCase()) {
						
						for (p = 5; p >= 3; p -= 1) {
							if (text[p]) {
								if (text[p].toLowerCase().indexOf("expire") > -1) {
									switch (text[p]) {
										case "Character Expired":
										case "Expires Today":
											print(name + " unperm");
											return false;
										default:
											days = +text[p].slice(11, -5);
		...
		print(name + " not found (unperm char check)");
		return true;
	},		
	
	//Return if account is safe or not (Chars expired) true/false
	isAccountSafe: function () {
		var control, text, string, days, p,
			list = [],
			charList = [],
			unperm = 0;
		...
		if (charList.length > list.length) { //1+ Chars are perm
			return true;
		}
		else {
			while (list.length > 0) {
