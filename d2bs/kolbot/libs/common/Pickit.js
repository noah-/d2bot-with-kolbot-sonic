...
var Pickit = {
	...
	checkItem: function (unit) {
		...
		// Gull Dagger fix
		if (unit.classid === 39 && unit.quality === 7) {
            return {
                result: 4,
                line: null
            };
		...
		if (NTIP.GetTier(unit) > 0 && [603, 604, 605].indexOf(unit.classid) !== -1) {
			//print("Charm: " + unit.name);
			return {
				result: NTIP.CheckItem(unit),
				line: "Inventory charm"
			};		
		}
	...
	pickItems: function () {
		...
		while (pickList.length > 0) {
			...
			if (copyUnit(pickList[0]).x !== undefined && (pickList[0].mode === 3 || pickList[0].mode === 5) &&
				...
				//print("Checking pickit for " + pickList[0].name + " result: " + status.result);
