...
var Item = {
	...
	canEquip: function (item, bodyLoc) {
		...
		var curr = this.getEquippedItem(bodyLoc);

		if (item.getStat(92) > me.getStat(12) || item.dexreq() > me.getStat(2) - curr.dex || item.strreq() > me.getStat(0) - curr.str) { // Higher requirements
			return false;
		}
		...
		for (i = 0; i < 3; i += 1) {
			if (item.toCursor()) {
				...
				if (item.bodylocation === bodyLoc) {
					//Misc.logItem("Equipped", item);
					if (getCursorType() === 3) {
						//Misc.click(0, 0, me);

						cursorItem = getUnit(100);

						//if (cursorItem && Pickit.checkItem(cursorItem).result > 0 && (NTIP.GetTier(cursorItem) < 1 || NTIP.GetTier(cursorItem) > 199 || this.autoEquipCheckMerc(cursorItem))) {
		...
		if (tier > 0 && bodyLoc) {
			for (i = 0; i < bodyLoc.length; i += 1) {
				...
				if (tier > oldTier && (this.canEquip(item) || !item.getFlag(0x10))) {
					print("\xFFc8Sonic \xFFc0:: New equip item: " + color + item.name + " \xFFc0(new: " + tier + ", old: " + oldTier + ")");
					return true;
				}
			}
		}
	...
	autoEquip: function () {
		...
		var i, j, tier, bodyLoc, tome, gid, that, classid,
		...
		function sortEq(a, b) {
			if (Item.canEquip(a) && Item.canEquip(b)) {
		...
		if (tier > 0 && bodyLoc) {
			for (i = 0; i < bodyLoc.length; i += 1) {
				...
				if (tier > oldTier && (this.canEquipMerc(item) || !item.getFlag(0x10))) {
					print("New merc item: " + color + item.name + " \xFFc0(new: " + tier + ", old: " + oldTier + ")");
					return true;
				}
			}
		}
	...
	townCheck: function () {
		...
		if (Config.TownCheck && !me.inTown) {
			try {
				...
				if (Config.OpenChests && Town.needKeys() && me.gold > 5000 && me.charlvl >= 19) {
					check = true;
				}
			} catch (e) {
				check = false;
		...
		if (check && !!Town.checkScrolls(518)) {
			print("townCheck");
...
var Packet = {
	...
	sellItem: function (unit) {
		if (unit.type !== 4) { // Check if it's an item we want to buy
			//D2Bot.printToConsole("packet.sellItem Error : unit is not item");
			print("packet.sellItem Error : unit is not item");
			return false;
