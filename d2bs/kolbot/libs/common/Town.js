...
CursorLoop:
	...
	buyKeys: function () {
		if (!this.wantKeys()) {
			//D2Bot.printToConsole("Don't want keys");
			return true;
		}
		...
		if (!npc) {
			D2Bot.printToConsole("buyKeys : No npc");
			return false;
		}
	...
	checkKeys: function () {
		if (!Config.OpenChests || me.classid === 6 || me.gold < 540 || (!me.getItem("key") && !Storage.Inventory.CanFit({sizex: 1, sizey: 1, gid:99999}))) {
			return 12;
		}
...
MainLoop:
	...
	needStash: function () {
		...
		var i, items = Storage.Inventory.Compare(Config.Inventory);
	...
	moveToSpot: function (spot) {
		var i, path, townSpot,
			...
			longRange = (me.classid === 1 && this.telekinesis && me.getSkill(43, 1) && ["stash"].indexOf(spot) > -1) || spot === "waypoint";
	...
	goToTown: function (act, wpmenu) {
		...
		if (!me.inTown) {
			if (!Pather.makePortal()) {
				...
				print("Town.goToTown: Failed to make TP");
				quit();
				delay(500);
			}
			...
			if (!Pather.usePortal(null, me.name)) {
				...
				print("Town.goToTown: Failed to take TP");
				quit();
				delay(500);
			}
		}
