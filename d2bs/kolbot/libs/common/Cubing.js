	...
	buildRecipes: function () {
		...
		for (i = 0; i < Config.Recipes.length; i += 1) {
			...
			case Recipe.Token:
				...
				break;
			case Recipe.PrismaticAmulet:
				this.recipes.push({Ingredients: [520, 561, 566, 571, 576, 581, 586], Index: Recipe.PrismaticAmulet});
...
IngredientLoop:
	...
	validItem: function (unit, recipe) {
		...
		if (recipe.Index === Recipe.PrismaticAmulet) {
	...
	doCubing: function () {
		...
		var i, j, items, string, result, tempArray,
			gems = [561, 566, 571, 576, 581, 586, 601];
				...
				if (items) {
					for (j = 0; j < items.length; j += 1) {
						...
						case 1:
							...
							if (gems.indexOf(items[j].classid) == -1) Misc.logItem("Cubing Kept", items[j], result.line);
