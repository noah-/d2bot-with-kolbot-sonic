...
MainLoop:
	...
	inGameCheck: function () {
		...
		if (this.torchAnniCheck === 2) {
			print("Ã¿c4AutoMuleÃ¿c0: In anni mule game.");
			D2Bot.updateStatus("AutoMule: In game.");
			this.dropCharm(true);
		} else if (this.torchAnniCheck === 1) {
			print("Ã¿c4AutoMuleÃ¿c0: In torch mule game.");
			D2Bot.updateStatus("AutoMule: In game.");
			this.dropCharm(false);
		} else {
			print("Ã¿c4AutoMuleÃ¿c0: In mule game.");
			D2Bot.updateStatus("AutoMule: In game.");
			this.dropStuff();
		}
	...
	dropStuff: function () {
		...
		D2Bot.printToConsole("AutoMule: Transfering " + items.length + " items.", 7);
        ...
        if (item) {
            do {
                if (Town.ignoredItemTypes.indexOf(item.itemType) === -1 &&
					...
					(me.profile.toLowerCase().indexOf("sonic") > -1 || item.location === 7 || (item.location === 3 && !Storage.Inventory.IsLocked(item, Config.Inventory))) &&
                    item.classid !== 549 && // Don't drop Horadric Cube
                    item.classid !== 524 && // ScrollOfInifuss
                    item.classid !== 525 && // KeyToTheCairnStones
                    item.classid !== 89 &&  // HoradricMalus
                    item.classid !== 552 && // HoradricMalus
                    item.classid !== 92 &&  // ShaftOfTheHoradricStaff
                    item.classid !== 521 && // TopOfTheHoradricStaff
                    item.classid !== 91 &&  // HoradricStaff
                    item.classid !== 546 && // AJadeFigurine
                    item.classid !== 547 && // TheGoldenBird
                    item.classid !== 545 && // PotionOfLife
                    item.classid !== 87 &&  // TheGidbinn
                    item.classid !== 548 && // Lamesen'sTome
                    item.classid !== 553 && // Khalim'sEye
                    item.classid !== 554 && // Khalim'sHeart
                    item.classid !== 555 && // Khalim'sBrain
                    item.classid !== 173 && // Khalim'sFlail
                    item.classid !== 174 && // Khalim'sWill
                    (item.classid !== 603 || item.quality !== 7) && // Don't drop Annihilus
                    (item.classid !== 604 || item.quality !== 7) && // Don't drop Hellfire Torch
                    ([603, 604, 605].indexOf(item.classid) !== -1 || item.location !== 3) && // Don't stash autoequip charms form inventory
                    // !this.cubingIngredient(item) && // Don't drop cubing ingredients
                    !this.runewordIngredient(item)) { // Don't drop Runeword ingredients
