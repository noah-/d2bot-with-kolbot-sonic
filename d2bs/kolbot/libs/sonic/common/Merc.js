...
function leftShift(val, shift) {
	while (shift >= bpe) {
		var l = Math.ceil((bitSize(val)+bpe-1)/bpe)+1;
		if (val.length < l)
			val = expand(val, l);
		leftShift_(val, bpe-1);
		shift -= bpe-1;
	}
	if (shift > 0) {
		var l = Math.ceil((bitSize(val)+bpe-1)/bpe)+1;
...
function rightShift(val, shift) {
	while (shift >= bpe) {
		rightShift_(val, bpe-1);
		shift -= bpe-1;
	}
	if (shift > 0)
		rightShift_(val, shift);
}

function Rnd(seed) {
	var tmp = int2bigInt(666, 16, 0);
	//print("constructor:");
	//print(bigInt2str(tmp,16));
	tmp = leftShift(tmp, 32);
	//print(bigInt2str(tmp,16));
	this.val = add(tmp, str2bigInt(seed.toString(16), 16, 33, 0));
	//var str = seed.toString(16);
	//while(str.length < 8)
	//	str = "0"+str;
	//print("29A"+str);
	//print(bigInt2str(this.val, 16));
}

Rnd.prototype.roll = function () {
	//var str = "roll:" + bigInt2str(this.val, 16);
	var tmp = modInt(this.val, 0x100000000);
	//str += " tmp:" + tmp.toString(16);
	var tmp2 = mult(str2bigInt(tmp.toString(16), 16, 33, 0), int2bigInt(1791398085, 33, 0));
	//str += " tmp2:"  + bigInt2str(tmp2, 16);
	rightShift(this.val, 32);
	//str += " val>>32:"  + bigInt2str(this.val, 16);
	var res = add(tmp2, this.val);
	var rescopy = dup(res);
	rightShift(rescopy, 64);
	tmp = leftShift(rescopy, 64);
	res = sub(res, rescopy);
	this.val = res;
	//str += " res:" + bigInt2str(res, 16);
	//print(str);
};

Rnd.prototype.get = function () {
	return modInt(this.val, 0x100000000);
};
